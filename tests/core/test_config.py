...
def test_create_and_load_default_config(tmp_path: Path, mocker):
    ...
    mocker.patch.object(
        config, "SYS_PROMPT_COMMIT_FILE", tmp_path / "system_prompt_commit.md"
    )
    mocker.patch.object(
        config, "USER_PROMPT_COMMIT_FILE", tmp_path / "user_prompt_commit.md"
    )
    ...
    assert (
        Path(loaded_cfg["prompt_paths"]["system_commit"]).name
        == "system_prompt_commit.md"
    )
...
def test_load_config_not_found(mocker):
    ...
    mocker.patch.object(
        config, "CONFIG_FILE", Path("/tmp/non_existent_dir/config.toml")
    )
